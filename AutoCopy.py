import pypdf  # import PyPDF2 module
...
def CollectQuestion(year):  # 定義函數回傳整份試題為字串
    reader = pypdf.PdfReader(year+'-Hema-Q.pdf')  # 讀取 PDF 檔案
    Data = ""  # 定義空字串
...
def ProcessAnswer(year):  # 定義函數回傳整份答案為 List
    Ans_reader = pypdf.PdfReader(year+'-Hema-A.pdf')  # 讀取 PDF 檔案
    Text = Ans_reader.pages[0].extract_text()  # 將每頁的文字轉成字串
    Header1 = Text.find("題號")  # 定義 Header1 為 "題號" 的位置
    Header2 = Text.find("\n備")  # 定義 Header2 為 "\n備" 的位置
    # 定義 Data1 為刪除 Header1 前和 Header2 後的字串（中間答案）
...
def BuildDataFrame(List, Data1):  # 建立 DataFrame
    ...
    for i in range(80):  # 80 題迴圈
        now_row = pd.Series([List[i], Data1[i], year], index=[
...
df1 = BuildDataFrame(ProcessQuestion(
    CollectQuestion(year)), ProcessAnswer(year))
